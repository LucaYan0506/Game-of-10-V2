...
class Game(models.Model):
    ...
    winner = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        # If a user account is deleted, we keep the game record but mark winner as null.
        on_delete=models.SET_NULL,
        # Access all games a user has won via user.games_won.all()
        related_name='games_won',
        # The game may not have a winner yet.
        null=True,
        blank=True
    )
    ...
    surrendered_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        # If the surrendering player's account is deleted, we keep the game history.
        on_delete=models.SET_NULL,
        # Access games a user surrendered via user.surrendered_games.all()
        related_name='surrendered_games',
        # This will be null for games that ended normally (not by surrender).
        null=True,
        blank=True,
    )
    ...
    def clean(self):
        ...
        if self.game_mode == self.GameMode.PVAI and all(self.ai_model not in model
                                                        for model in self.AiModel.choices):
